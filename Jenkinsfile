    ...
    stage ( 'Execute Ansible playbook' ) {
      steps {
        sshagent(['controlNode']) {
          ...
          sh 'ssh -tt -o StrictHostKeyChecking=no ubuntu@${ip} ansible-playbook -i inventory.aws_ec2.yml playbook.yml'
          sh 'sleep 20'
          sh 'ssh -tt -o StrictHostKeyChecking=no ubuntu@${ip} ansible-playbook -i inventory.aws_ec2.yml webserver.yml'
        }
      }
    }
  }
